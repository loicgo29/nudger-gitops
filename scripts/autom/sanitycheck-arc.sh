...
kubectl -n actions-runner-system get helmrelease actions-runner-controller

# Vérifie que le déploiement et les pods sont bien en place
echo "[3] Vérification du Deployment et Pods..."
kubectl -n actions-runner-system get deploy,pods -o wide

# Vérifie que le secret GitHub App existe
echo "[4] Vérification du Secret GitHub App..."
kubectl -n actions-runner-system get secret actions-runner-controller || echo "❌ Secret manquant"

# Vérifie le certificat TLS du webhook ARC
echo "[5] Vérification du certificat TLS du webhook..."
kubectl -n actions-runner-system get certificate actions-runner-controller-serving-cert || echo "❌ Certificat manquant"
