...
echo "[1] Service webhook dans actions-runner-system"
kubectl -n actions-runner-system get svc actions-runner-controller-webhook || echo "❌ Service manquant"
...
echo "[4] Pods ARC"
kubectl -n actions-runner-system get pods -l app.kubernetes.io/name=actions-runner-controller

# 6. Logs du manager (les 50 dernières lignes)
echo "[5] Logs du manager"
kubectl -n actions-runner-system logs deploy/actions-runner-controller -c manager --tail=50 || echo "⚠️ Impossible de récupérer les logs"
